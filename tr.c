	...
	rec = p->cgirec;
		...
		if ((name = rec->get_val(rec, PRO_DLD_XL_DOWNRATE)) != NULL) {
			...
			if((dspeed < 1) || (dspeed > 2048))
                ...
                limit.task_num = task_num;
                limit.task_co = task_co;
                limit.uspeed = uspeed;
                limit.dspeed = dspeed;
                limit.s = sockfd;
                
                pthread_create(&tid, NULL, (void*)tr_send_post_setlimit, &limit);
		...
		do{ 
			...
			if(s != NULL) {
                ...
                do{                       
                	s = rec->get_val_no(rec, PRO_XL_TASKS_ID, i);
			if (s != NULL) {
                                pthread_create(&tid, NULL, (void*)tr_send_post_remove, (void*)(atoi(s)));
                               	i++;
                        }
                        MCEMT_DBG("while\n")
                  }while(s);
