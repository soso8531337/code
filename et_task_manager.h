...
/*                    TYPE DEFINE                    */
/************************************************************************/
#ifndef __ET_TYPE_DEFINED__
#define __ET_TYPE_DEFINED__
...
/////3.28 开启高速通道
ETDLL_API int32 etm_open_high_speed_channel( u32 task_id );
 ...
 *  xlmv文件是否可以试播
 *  drm_id:			xlmv文件对应的drm_id
 *	p_can_preview:	表示是否可以试播
 * 返回值: 0     成功
           22534 无效的drm_id.
 */
ETDLL_API int32  etm_drm_file_can_preview(u32 drm_id, BOOL* p_can_preview);

/* 
 *  xlmv文件获取第一个试播段
 *  drm_id:      		xlmv文件对应的drm_id
 *  p_preview_bg_time:	试播段开始时间
 *  p_preview_end_time:	试播段终止时间
 * 返回值: 0     成功
           22534 无效的drm_id.
           22548 不支持试播.
 */
ETDLL_API int32  etm_drm_get_preview_time(u32 drm_id, int32* p_preview_begin_time, int32* p_preview_end_time);
 ...
 * 返回值: 0     成功
           ...
           22549 请求数据不在试播段内.
 */
ETDLL_API int32 etm_drm_read_preview(u32 drm_id, char *p_buffer, u32 size, uint64 file_pos, u32 *p_read_size);
...
#define ETM_OPENSSL_IDX_COUNT			    (8)
...
#define ETM_OPENSSL_RSA_PUBLIC_ENCRYPT_IDX  (7)
 ...
 *  序号：7 (ETM_OPENSSL_RSA_PUBLIC_ENCRYPT_IDX)  函数原型：typedef void(*et_func_BIO_free)( BIO *a );
 *  说明：对应openssl库的RSA_public_encrypt函数
